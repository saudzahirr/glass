      SUBROUTINE CGSSV(M, N, A, LDA, B, X, MAX_ITER, TOL, INFO)
C     ----------------------------------------------------------------
C     Gauss-Seidel solver for A * X = B (complex single, row-major)
C     ----------------------------------------------------------------
      INTEGER M, N, LDA, MAX_ITER, I, J, K, INDEX
      COMPLEX A(*), B(M), X(M), X_NEW(M)
      COMPLEX S1, S2
      REAL TOL, DIFF, MAX_DIFF
      INTEGER INFO
